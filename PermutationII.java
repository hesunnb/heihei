...
class Solution {
    ...
    private void permuteUniqueHelper(ArrayList<ArrayList<Integer>> result, ArrayList<Integer> list, ArrayList<Integer> nums, 
            boolean[] visit) {
        ...
        for(int i = 0; i < nums.size(); i++) {
            if(i != 0 && nums.get(i) == nums.get(i - 1) && !visit[i - 1]) { //这里的条件意思是i不等于0, //并且i处的值和它前面一位的值相等, 
