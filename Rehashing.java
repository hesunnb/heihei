...
C++/Java: if you directly calculate -4 % 3 you will get -1. You can use function: a % b = (a % b + b) % b to 
make it is a non negative integer.
...
public class Solution {
    ...
    public ListNode[] rehashing(ListNode[] hashTable) {
        ...
        for(int i = 0; i < hashTable.length; i++) {
            ...
            while(current != null) {
                if(current.val < 0) {
                    index = (current.val % result.length + result.length) % result.length;
                } else {
                    index = current.val % result.length;
                }
