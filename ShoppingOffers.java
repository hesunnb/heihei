...
class Solution {
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        if(price == null || price.size() == 0 || special == null || needs == null || needs.size() == 0) {
            return -1;
        }
    ...
    public int helper(List<Integer> price, List<List<Integer>> special, List<Integer> needs, int pos) {
        ...
        for(int i = pos; i < special.size(); i++) { //如果第一个special offer的所有组合都没能符合要求, 
            //那么从第二个开始的时候, 第一个以后的也用不上了, 所以用个pos, 可以加速(不用pos从0开始也行, 会稍微慢些)
            List<Integer> offer = special.get(i);
            List<Integer> tmp = new ArrayList<>();
            for(int j = 0; j < needs.size(); j++) {
                if(needs.get(j) < offer.get(j)) { //如果special里面的量多于needs, 就不能用special offer
                    tmp = null; //就把tmp赋为null
                    break;
                }
                tmp.add(needs.get(j) - offer.get(j)); //减掉special offer里面的量
            }
            
            if(tmp != null) {
                minCost = Math.min(minCost, offer.get(offer.size() - 1) + helper(price, special, tmp, i));
            }
        }
        return minCost;
    }
    
    public int directPurchase(List<Integer> price, List<Integer> needs) { //不用special offer, 全都单买的价格
        int total = 0;
