...
4 × 3 = 12
6 × 2 = 12
As you can see, calculations of 4 × 3 and 6 × 2 are not necessary. Therefore, we only need to consider factors up to √n 
...
#Hide Hint 4  
The Sieve of Eratosthenes is one of the most efficient ways to find all prime numbers up to n. But don't let that name 
scare you, I promise that the concept is surprisingly simple.
...
3 × 3 = 9, ... must not be primes, so we mark them off as well. Now we look at the next number, 4, which was already 
marked off. What does this tell you? Should you mark off all multiples of 4 as well?
...
#Hide Hint 6  
In fact, we can mark off multiples of 5 starting at 5 × 5 = 25, because 5 × 2 = 10 was already marked off by multiple of 
2, similarly 5 × 3 = 15 was already marked off by multiple of 3. Therefore, if the current number is p, we can always 
mark off multiples of p starting at p2, then in increments of p: p2 + p, p2 + 2p, ... Now what should be the terminating 
loop condition?
...
#Hide Hint 8  
Yes, the terminating loop condition can be p < √n, as all non-primes ≥ √n must have already been marked off. When the 
loop terminates, all the numbers in the table that are non-marked are prime.
...
class Solution {
    public int countPrimes(int n) {
        ...
        // Loop's ending condition is i * i < n instead of i < sqrt(n)
        // to avoid repeatedly calling an expensive function sqrt().
        for(int i = 2; i * i < n; i++) { //一直尝试到根号n即可
            if(isPrime[i] == false) { //已经被mark掉的直接跳过
                continue;
            }
            for(int j = i * i; j < n; j += i) { //从每个数的平方到n, 以这个数本身为间隔mark
                isPrime[j] = false;
            }
        }
        
        int count = 0;
        for(int i = 0; i < isPrime.length; i++) { //最后有多少个true就有多少个质数
            if(isPrime[i] == true) {
                count++;
            }
        }
