    ...
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        for(int i = 0; i < nums.length - 3; i++) { //i要保证后面有3个位置
            ...
            for(int j = i + 1; j < nums.length - 2; j++) { //j要保证后面有2个位置
                ...
                //比如-1,-1,-1 此时i,j是待匹配的头两个值, 匹配完后-1,-1的情况就都有了, 这是j再后窜一位还是-1, 那么待匹配的值还是-1,-1, 这就重复了, 所以要跳过
                //     i  j   
                    continue; //比如-1,-1,-1,-1,0,0,1,1,1,1,2(target=2) i,j在0,0处, 如果不加j != i + 1, 那么0,0,1,1这种情况就会continue而得不到
                    //                          i,j
                }
                int left = j + 1;
