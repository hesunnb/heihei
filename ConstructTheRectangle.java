...
But according to requirement 2, [1,4] is illegal; according to requirement 3,  [4,1] is not optimal compared to [2,2]. So the length L 
...
class Solution {
    public int[] constructRectangle(int area) {
        ...
        int width = (int)Math.sqrt(area); //先开方获得最接近的值
        while(area % width != 0) { //除不开的, 就让宽--, 减到能除开的时候就正好
            width--;
        }
        int length = area / width; //像area=2,6,9这种的直接都能过
        result[0] = length;
        result[1] = width;
        return result;
    }
