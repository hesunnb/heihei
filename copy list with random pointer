...
public class Solution {
    ...
    public RandomListNode copyRandomList(RandomListNode head) {
        ...
        while(head != null) //每个head节点要判断两次，一个自己在不在，一个自己的random指针指向的结点在不在
        {
            ...
            {
                newNode = new RandomListNode(head.label); //不在就建立一个新结点, 每新new一个实例会返回这个实例的地址覆盖掉原来
                //newNode的地址, 所以每次向哈希表里面放的是不同的地址
                map.put(head, newNode); //head也是一样, 每次都要head = head.next, head本身的值在不断发生变化, 所以放入哈希表的也是
                //不同的地址
            }
    ...
    private RandomListNode split(RandomListNode head)
    {
        ...
        while(head != null)
        {
            RandomListNode temp = head.next; //temp是个引用，并没有new
            head.next = temp.next;
            head = head.next;
            if(temp.next != null)
            {
