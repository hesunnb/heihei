...
class Solution {
    ...
    //从start位置开始左右跳, 进行递归, 看能不能跳到有0的位置
    ...
    public boolean canReachHelper(int[] arr, int start) {
        if(start < 0 || start >= arr.length || arr[start] >= arr.length) {
            return false;
        }
        int jump = arr[start];
        arr[start] += arr.length; //将访问过的点变为无效值, 不重复访问
        if(jump == 0 || canReachHelper(arr, start - jump) || canReachHelper(arr, start + jump)) {
            return true;
        }
        return false;
    }
